...
Map<String, dynamic> timeOfDayToMap(TimeOfDay timeOfDay) {
  return {
    'hour': timeOfDay.hour,
...
class YearlyTimeController extends GetxController {
...
class YearlyPage extends StatelessWidget {
  final YearlyTimeController timeController = Get.put(YearlyTimeController());
  TextEditingController headingController = TextEditingController();
  TextEditingController taskController = TextEditingController();
  TextEditingController yearController = TextEditingController();
  DateTime? selectedDate;

  CollectionReference yearlyTasksCollection =
      FirebaseFirestore.instance.collection('yearlyTasks');
...
void _toggleCompleteStatus(String? taskId, bool isComplete) {
  if (taskId != null) {
    try {
      ...
      final taskRef = FirebaseFirestore.instance.collection('yearlytasks').doc(taskId);
      taskRef.update({
...
void _editTask(BuildContext context, String? taskId) {
  YearlyTimeController timeController = Get.put(YearlyTimeController());
  ...
  void updateTask() async {
    final heading = updatedHeadingController.text;
    final task = updatedTaskController.text;
    final fromTime = updatedFromTime.value; // Use updated time values
    final toTime = updatedToTime.value; // Use updated time values
    ...
    if (taskId != null &&
      ...
      try {
        ...
        if (userId != null) {
          Map<String, dynamic> fromTimeMap = timeOfDayToMap(fromTime);
          Map<String, dynamic> toTimeMap = timeOfDayToMap(toTime);

          final taskCollection = FirebaseFirestore.instance.collection('yearlytasks');
          ...
          await taskCollection.doc(taskId).update({
            ...
            'task': task,
            'date': selectedDate,
            'fromTime': fromTimeMap,
            'toTime': toTimeMap,
          });

          // Clear the text fields and reset time fields
          updatedHeadingController.clear();
          updatedTaskController.clear();
          yearController.clear();
          timeController.setFromTime(null);
          timeController.setToTime(null);
          ...
          // Add any additional logic after task update here
        }
      } catch (e) {
  ...
  void selectYear(BuildContext context) async {
    ...
    if (datePicked != null) {
      selectedDate = datePicked;
      yearController.text = "${datePicked.year}-${datePicked.month}-${datePicked.day}";
    }
  }
              ...
              children: [
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: updateTask, // Call the function to update the task
                    child: const Text(
                      'Save Changes',
                      style: TextStyle(
                        color: Colors.white,
...
void _deleteTask(String? taskId) {
  if (taskId != null) {
    try {
      FirebaseFirestore.instance.collection('yearlytasks').doc(taskId).delete();
    } catch (e) {
